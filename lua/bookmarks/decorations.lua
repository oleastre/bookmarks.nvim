    ...
    for _, bmk in ipairs(bookmark_array) do
        if bmk.filename == current_filename and
            ...
            bmk.line <= vim.api.nvim_buf_line_count(bufnr) then
              ...
              Utils.debug_print(string.format("Placing sign at line %d", bmk.line))
            -- Place sign
            vim.fn.sign_place(
                0,
                "bookmarks_group",
            ...
            -- Add highlight using nvim_buf_add_highlight
            local line_idx = bmk.line - 1 -- Convert to 0-based index
